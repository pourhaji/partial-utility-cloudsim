 ...
 * @author Jose M. Simao
 */
public class VmSchedulerTimeSharedOverSubscription_SI_MaxVirtPwr_MinClass extends VmSchedulerTimeShared {
	 ...
	 */
	public VmSchedulerTimeSharedOverSubscription_SI_MaxVirtPwr_MinClass(List<? extends Pe> pelist) {
		super(pelist);
	}
	...
	protected boolean redistributeMipsDueToOverSubscription() {
		...
		while (allVMsInHost.size() != 0) {
			...
			for (int i=VMType.values().length-1; i>=0; --i)
		...
		if (!canFit) {
			Log.printLine(" *+*+* [SI_MaxVirtPwr.redistributeMipsDueToOverSubscription] Could not find VM to depreciate and fit " + needMIPS + " *+*+*");
			//System.exit(-1);
			return false;
		}
