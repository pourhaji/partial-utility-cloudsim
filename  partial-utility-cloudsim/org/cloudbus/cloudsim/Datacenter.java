...
public class Datacenter extends SimEntity {
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		...
		if (result) {
			...
			VmAllocationPolicy allocPolicy = getVmAllocationPolicy();
			Host h = allocPolicy.getHost(vm);
			VmScheduler sched = h.getVmScheduler();
			List<Double> mipsShare = sched.getAllocatedMipsForVm(vm);
			vm.updateVmProcessing(CloudSim.clock(), mipsShare);
		}
